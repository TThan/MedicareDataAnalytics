...
hive>: select sum(e.enrolled) as totalenrolled, c.plantype from enrollment e , contract c where c.planid = e.planid and e.contractid=c.contractid group by c.Plantype order by totalenrolled desc;

# Insight 4: Does the Medicare Advantage Plan offers Part D too?
hive>: select sum(e.enrolled) as totalenrolled, c.plantype, c.OffersPartD  from enrollment e , contract c where c.planid = e.planid and e.contractid=c.contractid group by c.Plantype, c.OffersPartD  order by totalenrolled desc;

# Insight 5: How many of the Medicare Advantage Plans offer Special Needs Plans too?
hive>select SNPPlan,count(snpplan) as snp  from contract group by SNPPlan;

# Insight 6: How many of the Medicare Advantage Plans offer Employers Group Health Plans too?
hive>  select eghp,count(eghp) as employergroup  from contract group by eghp;

# Insight 7: What are the top counties which have more enrollees for Medicare Advantage Plans?
hive> select sum(e.enrolled) as totalenrolled, e.county from enrollment e , contract c where c.planid = e.planid and e.contractid=c.contractid group by e.county order by totalenrolled desc limit 10;

# Insight 8: What are the Organizations that are favoured most for Medicare Advantage Plan?
hive> select sum(e.enrolled) as totalenrolled, c.ParentOrganization from enrollment e , contract c where c.planid = e.planid and e.contractid=c.contractid group by c.ParentOrganization order by totalenrolled desc limit 20;

# Insight 9: How are the number of enrollments for the different Medicare Advantage Plan types distributed among the states?
hive> select sum(e.enrolled) as totalenrolled, c.plantype, e.state from enrollment e , contract c where c.planid = e.planid and e.contractid=c.contractid group by c.Plantype, e.state order by totalenrolled desc;

# Insight 10: How are the Medicare Advantage Plan enrollees distributed among the states?
hive> select sum(e.enrolled) as totalenrolled, e.state from enrollment e , contract c where c.planid = e.planid and  e.contractid=c.contractid group by e.state order by totalenrolled desc;
